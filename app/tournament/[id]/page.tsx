...
export default function TournamentSetupPage() {
  ...
  if (!tournament) return <p className="text-white/40 text-center">Loading...</p>;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="glass rounded-xl p-4">
          <h2 className="font-semibold mb-3 text-white/80">Participants</h2>
          {participants.map((p) => (
        ...
        <div className="md:col-span-2 glass rounded-xl overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-white/10">
            <h2 className="font-semibold text-white/80">Teams ({teams.length})</h2>
            {isAdmin && (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="text-sm px-3 py-1 rounded-lg border border-white/20 text-white/70 hover:bg-white/10 transition-colors"
                >
                  Manual Entry
                </button>
              </div>
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {Object.entries(regionGroups).map(([region, rTeams]) => (
                    <div key={region}>
                      <h3 className="text-xs font-bold text-white/40 uppercase mb-2">
                        {region}
                      </h3>
                      {rTeams.map((t) => (
                        <div
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-strong rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">Manual Team Entry</h2>
              <button
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {Object.keys(regions).map((r) => (
                <div key={r}>
                  <label className="block text-sm font-bold mb-1 text-white/80">{r}</label>
                  <textarea
                    value={regions[r]}
                    onChange={(e) =>
